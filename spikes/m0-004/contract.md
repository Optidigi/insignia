...
The signer requires an explicit trusted shop-local issuance day D and signs only a `validThroughDay` E of D+2. Both full-set verifiers require the independently supplied current shop-local day in the inclusive interval E-2 through E, after checked date ordering. The wire payload does not carry D, so an offline Function cannot prove the physical signing moment or issuance history independently. This remains a trusted-issuer obligation until the principal adjudicates the candidate protocol. A complete set uses one key ID; two separate complete sets may be signed under old and new keys during overlap. The trusted public configuration bounds both bucket count and physical quantity. Marker presence is distinct from a missing marker value: a marked line without a token rejects. A zero market ID is permitted only when trusted configuration explicitly enables the no-market case.
...
An absent `m0_004_policy` projection on a product that should require authorization could make an unsigned line appear optional to the local Validation adapter. These synthetic fixtures cannot prove policy publication and propagation; live policy-projection and fail-closed rollout behavior must be established before enforcement reliance. This is a disclosed G6 blocker, not a local G6 pass.
