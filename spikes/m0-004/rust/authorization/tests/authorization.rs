...
fn strict_ed25519_rejects_weak_key_and_noncanonical_signature_scalar() {
    let t = token(claims(0, 1, 1, 3000, 1, 3000));
    assert_eq!(
        VerificationKey::new(9, [0; 32]).err(),
        Some(Error::Signature)
    );

    let k = keys();
    assert!(verify_set(&[line(&t, 111, 1, Some(3000))], &context(&k), true).is_ok());
    let mut raw = URL_SAFE_NO_PAD.decode(t).unwrap();
    let original_r = raw[114..146].to_vec();
    // Preserve valid R and add the group order L to valid S, so reduction modulo L would accept.
    let order_l = [
    ...
    for (i, byte) in order_l.iter().enumerate() {
        let sum = raw[146 + i] as u16 + *byte as u16 + carry;
        raw[146 + i] = sum as u8;
        carry = sum >> 8;
    }
    assert_eq!(carry, 0);
    assert_eq!(&raw[114..146], original_r);
    let bad = URL_SAFE_NO_PAD.encode(raw);
    assert_eq!(
        verify_set(&[line(&bad, 111, 1, Some(3000))], &context(&k), true),
        Err(Error::Signature)
...
fn identity_key_identity_r_zero_s_is_rejected_by_pinned_strict_verifier_and_admission() {
    ...
    assert_eq!(
        VerificationKey::new(9, identity).err(),
        Some(Error::Signature)
    );
    assert_eq!(
        verify_set(
...
fn key_rotation_accepts_separate_sets_but_rejects_mixed_keys_within_one_set() {
    ...
    let old_key = keys()[0];
    let rotated_keys = [old_key, VerificationKey::new(10, old_key.bytes()).unwrap()];
