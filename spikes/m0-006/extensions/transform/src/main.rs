...
fn cart_transform_run(input: schema::run::CartTransformRunInput) -> Result<Output> {
    ...
    if config.max_buckets > MAX_EXPERIMENT_BUCKETS {
        return Ok(empty());
    }
...
mod tests {
    ...
    #[test]
    fn ten_bucket_output_is_bounded_and_oversized_sets_emit_no_partial_operation() {
        for (name, signed) in [
            ("10-signed-190-ordinary", 10),
            ("32-signed-0-ordinary", 0),
