...
The user owns product decisions. The principal architect/reviewer is ChatGPT in the Insignia Rewrite Project. The local orchestrator is sol-6-high. Implement only the authorized slice; return every PR for principal review. The current authorization is **M0-003** in `docs/delivery/prompts/M0-003-fulfillment-selection-diagnosis.md`, following [the principal's attributed review of PR #4](docs/delivery/PR-004-principal-review.md) and the user's exact-PR merge and bounded diagnostic delegation. M0-003 preserves order #1001 and diagnoses the intended-one/actual-two native fulfillment observation. Its pre-submit form already reported two selected items, so the cause remains undetermined. G1 remains IN_PROGRESS; G2–G8 remain NOT_RUN. Preserve the historical failure and stop any new fulfillment when form state or outgoing request cannot be reconciled.
