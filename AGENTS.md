...
The user owns product decisions. The principal architect/reviewer is ChatGPT in the Insignia Rewrite Project. The local orchestrator is sol-6-high. Implement only the authorized slice; return every PR for principal review. The current authorization is **M0-004R** in `docs/delivery/prompts/M0-004R-correctness-and-capacity.md`, following [the principal's attributed external CHANGES_REQUESTED review of PR #7](docs/delivery/PR-007-principal-review.md). Correct existing PR #7 with off-store protocol regressions and bounded same-protocol capacity evidence. G1 remains IN_PROGRESS with limited C/R/B acceptance, its immediate-R capture gap and #1001 cause UNDETERMINED. G2/G3/G5 remain IN_PROGRESS; no gate pass or protocol freeze is authorized. No Shopify mutation or new merchant credential is authorized for this slice. Use the actual high-effort orchestrator route and return the corrected PR #7 for principal review without merging it.
