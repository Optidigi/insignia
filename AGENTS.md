...
The user owns product decisions. The principal architect/reviewer is ChatGPT in the Insignia Rewrite Project. The local orchestrator is sol-6-high. Implement only the authorized slice; return every PR for principal review. The current authorization is **M0-003R version 1.1** in `docs/delivery/prompts/M0-003R-native-outcome-continuation.md`, following [the principal's attributed external review of PR #5](docs/delivery/PR-005-principal-review.md) and the user's exact-PR merge and bounded staging delegation. M0-003R preserves order #1001 and tests fresh native control/candidate outcomes. The original cause remains undetermined. G1 remains IN_PROGRESS; G2–G8 remain NOT_RUN. Before a new fulfillment, establish coherent native line identity, effective quantities and aggregate from fresh direct observations. The prompt makes native request-body capture optional; an invariant mismatch still stops the dependent action.
