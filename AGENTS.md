...
The user owns product decisions. The principal architect/reviewer is ChatGPT in the Insignia Rewrite Project. The local orchestrator is sol-6-high. Implement only the authorized slice; return every PR for principal review. The current authorization is **M0-005** in `docs/delivery/prompts/M0-005-whole-quote-authorization-feasibility.md`, following the [principal's external approval of PR #7](docs/delivery/PR-007R-principal-review.md) and its exact normal merge. Prototype one whole-quote Ed25519 authorization in `spikes/m0-005/`; retain the approved plan, ledger and M0-004 protocol/source/evidence. The candidate is a proposal, not an adopted architecture. G1 remains IN_PROGRESS with limited C/R/B acceptance, its immediate-R capture gap and #1001 cause UNDETERMINED. G2/G3/G5 remain IN_PROGRESS; no gate pass or protocol freeze is authorized. This slice is off-store: no merchant API call or Shopify mutation. Return one M0-005 outcome PR for principal review without merging it.
