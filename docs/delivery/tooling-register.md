...
| Codex client model/effort | VERIFIED for explicit launch | CLI 0.156.1 app-server `config/read --strict-config` resolved `gpt-6-sol`/`high` from session flags; fresh no-tool CLI response succeeded without reported fallback. Plain project config loading remains unverified; future sessions must use explicit flags until repaired. No provider-private attestation claimed. |
...
| Shopify existing app | VERIFIED identity; distribution UNVERIFIED | CLI existing-app import in private scratch bound `insignia` in org `212732011` to OAuth client ID `942e6668fd1177524c0fc48b104b0ac3`. Requested scopes empty. Read-only `app execute` reports app not installed on `insignia-staging.myshopify.com`; no Admin API access. Neither CLI nor user-observed Dashboard control exposes distribution status yet. |
...
| Shopify authenticated store metadata | VERIFIED | CLI account login succeeded; `store list` and `store info` read `insignia-staging.myshopify.com`, shop `gid://shopify/Shop/78935261342`, type dev, plan basic. Direct `store auth` sessions remain empty. This does not prove app installation or Admin API access. |
| Shopify app identity/distribution | MISSING | User named app `insignia` and supplied Dashboard app URL under organization `212732011`; page inaccessible to connected web tool and CLI `app info` requires an app project. No app config was created. Name/client ID, distribution, scopes and install state not independently verified. |
