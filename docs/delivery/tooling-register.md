...
| Shopify existing app | Named app/client ID VERIFIED; Dashboard numeric binding and distribution UNVERIFIED | CLI existing-app import in private scratch bound `insignia` in org `212732011` to OAuth client ID `942e6668fd1177524c0fc48b104b0ac3`. It did not return Dashboard resource `427859050497` or distribution. Requested scopes empty. Read-only `app execute` reports app not installed on `insignia-staging.myshopify.com`; no Admin API access. User reports no Distribution section on app Home. |
