...
| Test resources | Existing `insignia` app, OAuth client ID `942e6668fd1177524c0fc48b104b0ac3`, organization `212732011`; owner-attested Dashboard resource `427859050497`. Only `insignia-staging.myshopify.com`, shop ID `gid://shopify/Shop/78935261342`, Basic development store, USD. App-specific API confirms installed app ID `gid://shopify/AppInstallation/781307904158`, granted scopes and active dev preview; exact owned fixture and transform IDs are in G1 evidence. Distribution remains UNVERIFIED. |
| Gate evidence | [G1 same-variant spike](../../spikes/evidence/G1.md) is IN_PROGRESS. Local 2026-07 schema/Function tests run; no full G1 PASS can follow from a development store. |
| Next review | One M0-001 implementation/evidence PR; principal verdict PENDING. Do not merge M0-001 or start another slice. |

The staging operator must record owned transform, preview, fixture product, inventory and order IDs before cleanup. No distribution selection or app deploy is authorized. PostgreSQL, R2 and billing remain later prerequisites outside this DB-free spike.
