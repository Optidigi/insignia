...
| Shopify test context | CLI imported the named existing `insignia` app in private scratch and returned OAuth client ID `942e6668fd1177524c0fc48b104b0ac3`. The owner confirmed the same Client ID under Settings → Credentials at Dashboard resource `427859050497`; that URL binding is owner-attested, while CLI app/client ID association is independently checked. `insignia-staging.myshopify.com` remains the verified Basic dev store. Read-only app Admin query reported **not installed**; requested scopes are empty. Distribution is **UNVERIFIED**: CLI omits it and the owner reports no Distribution section. |
...
The app owner may supply a narrow distribution-status read if one becomes available. The user relays the eventual PF-002 PR and exact head to the principal. No application scaffold, deployment, store mutation, production action or gate work occurred.
