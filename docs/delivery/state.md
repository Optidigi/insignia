...
| Shopify test context | CLI imported the existing `insignia` app in private scratch and returned OAuth client ID `942e6668fd1177524c0fc48b104b0ac3`. `insignia-staging.myshopify.com` remains the verified Basic dev store. Read-only app Admin query reported **not installed**; requested scopes are empty. Distribution is **UNVERIFIED**: CLI does not expose it and user reports no setting control in the Dashboard. |
...
The app owner can supply only a nonsecret read-only distribution status if no supported CLI read exists. The user relays the eventual PF-002 PR and exact head to the principal. No application scaffold, deployment, store mutation, production action or gate work occurred.
