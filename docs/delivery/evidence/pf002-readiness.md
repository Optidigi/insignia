...
try:
    ...
    bad_statuses = {'failed', 'error', 'cancelled'}
    surfaced_failures = []
    for event in all_events:
        ...
        errors = (event.get('error'), params.get('error'), item.get('error'), turn_data.get('error'))
        reason = str(params.get('reason') or '').lower()
        if (method.endswith(('/failed', '/error')) or
            'fallback' in method or 'fallback' in reason or any(errors) or
            any(str(value).lower() in bad_statuses for value in (params.get('status'), item.get('status'), turn_data.get('status'))) or
            (item.get('type') == 'commandExecution' and item.get('status') == 'completed' and item.get('exitCode') != 0)):
            surfaced_failures.append(method)
    assert not surfaced_failures, surfaced_failures
finally:
    proc.terminate()
    try:
        proc.wait(timeout=3)
