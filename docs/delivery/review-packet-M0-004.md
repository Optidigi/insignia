...
| Canonical vectors and amount/set tests | `cd spikes/m0-004 && ./scripts/check-local.sh` in isolated tool environment | PASS: 57 TS; 15 Rust behavior + 2 cross-language; 1 native test per target; fmt/Clippy | [Verification manifest](../../spikes/m0-004/evidence/verification.json), [vector corpus](../../spikes/m0-004/fixtures/vectors.json) |
...
| Historical local G1 suite | `cd spikes/m0-001 && corepack pnpm test` with project-local Rust/CLI environment | PASS: exit 0, 11 fixtures; no live staging run | [Retained output](../../spikes/m0-004/evidence/historical-m0-001-test.log), [digest and environment](../../spikes/m0-004/evidence/verification.json) |
...
Two fresh restricted, read-only `gpt-6-sol`/high sessions reviewed the first integrated candidate `db0e260db20925a4e6af2c3d5f09c09614ac0861` against the verified merge base. Spec session `01a0d95e-ab2b-74d2-b168-3b40b2d72bce` found a zero-variant TS/Rust mismatch, missing Rust physical-quantity cap and untested allocation-to-issuance seam. Standards/correctness session `01a0d95e-ab30-7140-be90-f3233d5191b9` found raw high-bit currency/country decode mismatch, unbounded issuance validity, mixed-key sets and a missing marked-line distinction in TypeScript. The corrections and adversarial tests are in this candidate. Fresh final-diff Spec session `01a0d96b-7636-7190-930c-5ee221d6384f` confirmed those seven findings closed and retained the policy-projection enforcement risk. Fresh Standards/correctness session `01a0d96b-7623-70f3-88f3-f4e5191c97c8` found two additional codec/parser defects: Rust accepted a bucket whose declared quote quantity or total was smaller than the bucket, and a non-ASCII hex string could panic at a byte slice. Both are corrected with Rust regression tests. That reviewer also requested a reproducible historical fixture receipt; the exact command, result, retained output and digest are in the verification manifest. Both reviewers upheld the disclosed instruction-limit failure and unverified live Validation semantics; neither gave gate or PR approval.

The payload has no issuance-day field, so the Function cannot prove D from the wire even though the signer enforces D+2. An absent policy projection on a product that requires authorization could make an unsigned line appear optional. Those are retained limits for principal and live rollout adjudication. Fresh final-diff follow-up session identities and limits are recorded in the PR metadata. No reviewer granted PR or gate approval.
